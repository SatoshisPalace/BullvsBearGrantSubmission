...
static CONTEST_ID_STORE: AppendStore<String> = AppendStore::new(b"Contest_Ids");

// Function to get a user-specific AppendStore
fn get_all_contest_id_store() -> AppendStore<'static, String> {
    let user_store = CONTEST_ID_STORE.add_suffix(b"all_contests");
    return user_store;
}

pub fn get_all_contest_ids(storage: &dyn Storage) -> StdResult<Vec<String>> {
...
pub fn add_contest_id(storage: &mut dyn Storage, contest_id: &String) -> StdResult<()> {
    let contest_store = get_all_contest_id_store();
