#[cfg(test)]
pub mod tests{
	use cosmwasm_std::{testing::{mock_env, MockStorage, MockApi, MockQuerier, mock_dependencies}, OwnedDeps, Empty, from_binary, Uint128, Addr, Binary, StdResult};

	use crate::{msg::{ExecuteMsg, QueryMsg}, contract::query, contest::{response::UserBetQueryResponse, data::bets::UserContest}, tests::{contract_init_test::tests::_initialize_test, create_contest_test::tests::{_get_valid_create_contest_msg, _create_contest_test}, viewing_key_test::tests::_set_viewing_key_test, bet_contest_test::tests::{_bet_contest_test, _get_valid_bet_contest_msg}}};
	...
	fn query_bet_after_contest_creation(){
		...
		if let ExecuteMsg::CreateContest { contest_info, contest_info_signature_hex: _, outcome_id: _, sender: _, amount } = msg {
	...
	fn query_bet_bad_viewing_key(){
		...
		if let ExecuteMsg::CreateContest { contest_info, contest_info_signature_hex: _, outcome_id: _, sender: _, amount: _ } = msg {
	...
	fn _query_bet_after_betting_twice_test(
		...
		let (first_bet, contest_id) = if let ExecuteMsg::CreateContest { contest_info, amount, .. } = create_contest_msg {
